...
use Data443\LGPD\DataSubject\DataSubjectIdentificator;
use Data443\LGPD\DataSubject\DataSubjectManager;
use Data443\LGPD\DataSubject\DataExporter;
use Data443\LGPD\Components\Consent\UserConsentModel;
 ...
 *
 * @package Data443\LGPD\Components\DataPage
 */
class PrivacyToolsPageController {
	...
	protected function setup() {
		...
		add_action( 'lgpd/frontend/action/identify', array( $this, 'sendIdentificationEmail' ) );

		add_action( 'lgpd/frontend/privacy-tools-page/content', array( $this, 'renderConsentForm' ), 10, 2 );
		add_action( 'lgpd/frontend/privacy-tools-page/content', array( $this, 'renderExportForm' ), 20, 2 );

		add_action( 'lgpd/frontend/privacy-tools-page/content', array( $this, 'renderDeleteForm' ), 30, 2 );

		add_action( 'lgpd/frontend/privacy-tools-page/action/withdraw_consent', array( $this, 'withdrawConsent' ), 10, 2 );
		add_action( 'lgpd/frontend/privacy-tools-page/action/export', array( $this, 'export' ), 10, 2 );
		add_action( 'lgpd/frontend/privacy-tools-page/action/forget', array( $this, 'forget' ), 10, 2 );
	...
	public function enqueue() {
		if ( ! lgpd( 'options' )->get( 'enable_stylesheet' ) || ! is_page( lgpd( 'options' )->get( 'tools_page' ) ) ) {
			return;
		}

		wp_enqueue_style(
			'lgpd-framework-privacy-tools',
			lgpd( 'config' )->get( 'plugin.url' ) . 'assets/css/privacy-tools.css'
		);
		wp_enqueue_script(
			'donot-sell-form',
			// Insert here your JavaScript file URL,
			lgpd( 'config' )->get( 'plugin.url' ) . 'assets/js/lgpd-donotsell.js',
			array( 'jquery' ),
			LGPD_FRAMEWORK_VERSION,
			true
		);
		wp_localize_script(
			'donot-sell-form',
    ...
    public function sendIdentificationEmail() {
        ...
        if ( ! is_email( $_REQUEST['email'] ) ) {
            $this->redirect( array( 'lgpd_notice' => 'invalid_email' ) );
        } else {
        ...
        if ( $this->dataSubjectIdentificator->isDataSubject( $requested_email ) ) {
            ...
            if ( empty($user) ) {
                $this->redirect( array( 'lgpd_notice' => 'unregistered_user' ) );
            } else {
                $this->dataSubjectIdentificator->sendNoDataFoundEmail( $requested_email );
            }
        }

        $this->redirect( array( 'lgpd_notice' => 'email_sent' ) );
    }
	...
	protected function renderNotices() {
		...
		echo lgpd( 'view' )->render( 'privacy-tools/notices' );
	}
	...
	protected function renderIdentificationForm() {
		$nonce = wp_create_nonce( 'lgpd/frontend/action/identify' );
		// FRAM-147 Fix reference of an undefined variable 'notices'
		if (!isset($notices)) {
			$notices = "NOTICES PLACEHOLDER";
		}
		echo lgpd( 'view' )->render( 'privacy-tools/form-identify', compact( 'nonce', 'notices' ) );
	}
	...
	protected function renderPrivacyTools( DataSubject $dataSubject ) {
		...
		echo lgpd( 'view' )->render( 'privacy-tools/privacy-tools', compact( 'dataSubject', 'email' ) );
	}
			...
			foreach ( $consentData as &$item ) {
				$item['withdraw_url'] = add_query_arg(
					array(
						...
						'lgpd_nonce'  => wp_create_nonce( 'lgpd/frontend/privacy-tools-page/action/withdraw_consent' ),
						'email'       => $dataSubject->getEmail(),
						'consent'     => $item['slug'],
					)
				);
	...
	public function renderExportForm( DataSubject $dataSubject ) {
		$email = $dataSubject->getEmail();
		$nonce = wp_create_nonce( 'lgpd/frontend/privacy-tools-page/action/export' );
	...
	public function renderDeleteForm( DataSubject $dataSubject ) {
		...
		if ( current_user_can( 'manage_options' ) ) {
			echo lgpd( 'view' )->render( 'privacy-tools/notice-admin-role' );
			return;
		}
		$email     = $dataSubject->getEmail();
		$lgpd_user = get_user_by( 'email', $email );
		if ( isset( $lgpd_user->data->ID ) ) {
			if ( user_can( $lgpd_user->data->ID, 'manage_options' ) ) {
				echo lgpd( 'view' )->render( 'privacy-tools/notice-admin-role' );
				return;
			}
		}
		$action = 'forget';
		$nonce  = wp_create_nonce( 'lgpd/frontend/privacy-tools-page/action/forget' );
	...
	public function export( DataSubject $dataSubject ) {
		$format = sanitize_key( $_REQUEST['lgpd_format'] );
	...
	public function forget( DataSubject $dataSubject ) {
		...
		if ( $deleted ) {
			...
			$this->redirect( array( 'lgpd_notice' => 'data_deleted' ) );
		} else {
	...
	protected function redirect( $args = array(), $baseUrl = null ) {
		if ( ! $baseUrl ) {
			...
			if ( lgpd( 'options' )->get( 'custom_tools_page' ) ) {
				...
				$baseUrl         = apply_filters( 'redirect_after_lgpd_submit', $privacyToolsUrl );
			} else {
				$privacyToolsUrl = lgpd( 'options' )->get( 'tools_page' );
				$baseUrl         = $privacyToolsUrl ? get_permalink( $privacyToolsUrl ) : home_url();
				$baseUrl         = apply_filters( 'redirect_after_lgpd_submit', $baseUrl );
			}
