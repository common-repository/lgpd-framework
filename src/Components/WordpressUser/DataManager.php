...
class DataManager {
	...
	public function getData( DataSubject $dataSubject ) {
		...
		*/
		...
		return apply_filters( 'lgpd/wordpress-user/export/data', $data );
	}

	public function deleteUser( DataSubject $dataSubject, $reassign = null ) {
		require_once ABSPATH . 'wp-admin/includes/user.php';

		$reassignOption = lgpd( 'options' )->get( 'delete_action_reassign' );
		if ( 'reassign' === $reassignOption ) {
			$reassignUserId = lgpd( 'options' )->get( 'delete_action_reassign_user' );
		} else {
			$reassignUserId = false;
		}
		if ( $dataSubject->getUserId() ) {
			$dataSubject->lgpd_delete_log( $dataSubject->getUserId() );
	...
	public function anonymizeUser( DataSubject $dataSubject, $anonymizedId ) {
		...
		update_user_meta( $dataSubject->getUserId(), 'lgpd_anonymized_id', $anonymizedId );

		// Change username
		global $wpdb;

		$anonymizedUsername = apply_filters( 'lgpd/wordpress-user/anonymize/username', '[anonymous]' );
		$wpdb->update(
			$wpdb->users,
		...
		if ( apply_filters( 'lgpd/wordpress-user/anonymize/change_role', true ) && get_role( 'anonymous' ) ) {
