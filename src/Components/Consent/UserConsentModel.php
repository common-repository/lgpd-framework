...
class UserConsentModel {
	...
	/* @var string */
	public $version = LGPD_FRAMEWORK_VERSION;
	...
	protected function setTableName() {
		global $wpdb;
		$this->tableName = $wpdb->prefix . 'lgpd_consent';
	}
	...
	public function savelog( $email, $userlog ) {
		...
		$this->savelog_lgpd( $email, $userlog );
	}
	 ...
	 * @param $userlog
	 */
	protected function savelog_lgpd( $user_id, $userlog ) {
		global $wpdb;
		if ( ! empty( $_SERVER['HTTP_CLIENT_IP'] ) ) {
			$ip = $_SERVER['HTTP_CLIENT_IP'];
		} elseif ( ! empty( $_SERVER['HTTP_X_FORWARDED_FOR'] ) ) {
