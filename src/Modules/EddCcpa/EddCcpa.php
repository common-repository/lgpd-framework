<?php
namespace Data443\LGPD\Modules\EddCcpa;
...
class EddCcpa {
	...
	public function __construct( DataSubjectManager $dataSubjectManager, ConsentManager $consentManager ) {
		...
		if ( ! lgpd( 'options' )->get( 'enable_edd_compatibility' ) ) {
			return;
		}
		...
		add_filter( 'lgpd/data-subject/data', array( $this, 'getEddExportData' ), 20, 2 );
		add_action( 'lgpd/data-subject/delete', array( $this, 'deleteEddEntries' ) );
		add_action( 'lgpd/data-subject/anonymize', array( $this, 'anonymizeEddEntries' ) );
		if ( $show_agree_to_privacy > 0 ) {
			add_action( 'edd_complete_purchase', array( $this, 'edd_lgpd_consent' ) );
		}
	}
	...
	public function getEddExportData( array $data, $email ) {
		...
		if ( function_exists( 'edd_privacy_customer_record_exporter' ) ) {
			...
			$title               = __( 'EDD Customer Information', 'lgpd-framework' );
		...
		if ( function_exists( 'edd_privacy_billing_information_exporter' ) ) {
			...
			$ordertitle          = __( 'EDD Order Information', 'lgpd-framework' );
		...
		if ( function_exists( 'edd_privacy_file_download_log_exporter' ) ) {
			...
			$ordertitle          = __( 'EDD File Downloads', 'lgpd-framework' );
		...
		if ( function_exists( 'edd_privacy_api_access_log_exporter' ) ) {
			...
			$ordertitle          = __( 'EDD API Access Logs', 'lgpd-framework' );
	...
	*/
	public function deleteEddEntries( $email ) {
		$this->edd_lgpd_delete_orders( $email );
	}
	/*
	...
	*/
	private function edd_lgpd_delete_orders( $email ) {
		/*
	...
	/*
	*   Add checkout LGPD content
	*/
	public function edd_lgpd_consent( $payment_id ) {
